    ...
    <div className="big-two-login-wrapper">
      ...
      <div className="pin-board">
        { state.code.split('').map( (x: string, i: number) => {
          return <span key={`pin-card-${i}`} className={classNames('pin-card', {
        ...
        <input type="tel" id="big-two-login-pin" maxLength={state.code.length} value={state.value} onChange={onChange} onFocus={onFocus} onBlur={onBlur} />
      </div>
      {!!state.error && <div aria-live="polite" className="input-error">{state.error}</div>}
    </div>
  );
}
